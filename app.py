...
def load_faiss_index():
    _embedder  = SentenceTransformer("all-MiniLM-L6-v2")  # loads from cache instantly
    embeddings = _embedder.encode(KNOWLEDGE_BASE, convert_to_numpy=True)
...
with st.spinner("⏳ Loading models (first run only — may take 1-2 mins)..."):
    ...
    faiss_idx = load_faiss_index()  # no parameter
...
##cd C:\Users\Amrwa\Downloads
##C:\Users\Amrwa\miniconda3\Scripts\streamlit.exe run app.py